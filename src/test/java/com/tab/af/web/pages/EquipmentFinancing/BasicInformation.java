...
public class BasicInformation extends BasePage {
	...
	try {
	    ...
	    report.reportHardFail(e, e.getMessage());
	}
    }

    public BusinessInformation fillBasicInformation(String legalEntityType, String businessAs,
	    String secondApplicantType) {
	...
	if (legalEntityType.equalsIgnoreCase("Sole proprietor")) {
		...
		report.reportHardFail(e, "Fill Basic Information");
	    }
	}
	...
	try {
	    ...
	    shadow.sendKeys(txt_mobile, map.get("bestphone"), "Mobile");
	    shadow.sendKeys(txt_businessPhone, map.get("bestphone"), "Business");
	    shadow.sendKeys(txt_yearsDriving, faker.random().nextInt(1, 5).toString(), "Business");
	    shadow.click(btn_Next, "Next");
	    report.reportPass("Fill Sole Properiotor Basic Information");
	} catch (Exception e) {
	    report.reportHardFail(e, "Fill Sole Properiotor Basic Information");
	}
    }
}
