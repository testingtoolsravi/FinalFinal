...
public class CertificateOfDeposit extends BasePage {
	...
	try {
	    ...
	    report.reportHardFail(e, "Assert HomePage");
	}
    }
	...
	try {
	    ...
	    shadow.sendKeys(txt_FirstName, faker.name().firstName(), "First Name");
	    shadow.sendKeys(txt_LastName, faker.name().lastName(), "Last Name");
	    shadow.sendKeys(txt_PhysicalAddress, faker.address().streetAddress(), "Physical Address ");
	    shadow.sendKeys(txt_PhysicalAddressCity, faker.address().cityName(), "Physical Address City");
	    shadow.sendKeystoAuto(drp_PhysicalAddressState, faker.address().state(), "Physical Address State");
	    shadow.sendKeys(txt_PhysicalAddressZIP, faker.address().zipCode(), "Physical Address City");
	    shadow.sendKeys(txt_PersonalPhone, map.get("PHONE"), "Personal Phone");
	    webActions.sendKeys(shadow.findElements(txt_SSN).get(0), faker.number().digits(9), "SSN");
	    ...
	    report.reportHardFail(e, "Pay on Death");
	}
