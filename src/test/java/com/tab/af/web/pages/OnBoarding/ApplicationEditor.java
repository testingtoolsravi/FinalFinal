...
public class ApplicationEditor extends BasePage {
	...
	try {
	    ...
	    report.reportHardFail(e, "Select Created Application Failed");
	}
    }
	...
	try {
	    ...
	    webActions.waitUntilPresentAndDisplayed(product, selectedProduct);
	    ...
	    report.reportHardFail(e, "Select Product: Associated with Created Application Failed");
	}
    }

    public void startDepositProcessFromHomePage() {
	try {
	    String selectedProduct = (String) getTestDataMap().get("SelectedProduct");
	    WebElement product = webActions.findByText(selectedProduct, "//td//div");
	    ...
	    webActions.waitUntilPresentAndDisplayed(lbl_ProcessStartedSuccessfully, "Process SuccessFully Started");
	    report.reportPass("Start Process From HomePage for Created Application");
	} catch (Exception e) {
	    report.reportHardFail(e, "Start Process From HomePage for Created Application");
	}
    }

    public void startLoanProcessFromHomePage() {
	try {
	    String selectedProduct = (String) getTestDataMap().get("SelectedProduct");
	    WebElement product = webActions.findByText(selectedProduct, "//td//div");
	    webActions.waitForStalenessOf(product, selectedProduct);
	...
	try {
	    ...
	    webActions.waitUntilPresentAndDisplayed(lbl_DeclineApplication, "Decline Application");
	    webActions.clickBYJS(lbl_DeclineReasonOne, "Decline Reason Checkbox one");
	    webActions.sendKeysByChar(txt_DeclineComment, faker.lorem().characters(10), "Decline Comment");
	    webActions.clickBYJS(btn_OK, "OK");
	    webActions.waitUntilNotPresent(btn_OK, "OK");
	    webActions.waitUntilPresentAndDisplayed(lbl_ApplicationSuccessfullyDenied,
		    "Application Denied Successfully");
	    report.reportPass("Decline Application for  product Associated with Created Application");
	} catch (Exception e) {
	    report.reportHardFail(e, "Decline Application for product Associated with Created Application");
	}
    }
	...
	try {
	    ...
	    webActions.waitUntilEnabledCubaButton(btn_Decline, "Decline");
	    webActions.clickBYJS(btn_Decline, "Decline");
	    webActions.waitUntilPresentAndDisplayed(lbl_ApplicationDeclineInitialized,
		    "Application Decline Initialized");
	    report.reportPass("Decline Application for  product Associated with Created Application");
	} catch (Exception e) {
	    report.reportHardFail(e, "Decline Application for product Associated with Created Application");
	}
	...
	try {
	    ...
	    report.reportHardFail(e, "WithDraw Application for product Associated with Created Deposit Application");
	}
    }
	...
	try {
	    ...
	    report.reportHardFail(e, "WithDraw Application for product Associated with Created Loan Application");
	}
    }
	...
	webActions.waitUntilPresentAndDisplayed(lbl_WithdrawApplication, "withdraw Application");
	webActions.SendKeysByCharAuto(lbl_WithDrawReason, "Other", "withdraw Reason");
	webActions.sendKeysByChar(txt_WithdrawComment, faker.lorem().characters(5), "Withdraw Comment");
	webActions.clickBYJS(btn_OK, "OK");
	webActions.waitUntilNotPresent(btn_OK, "OK");
	webActions.waitUntilPresentAndDisplayed(lbl_ApplicationSuccessfullyWithDrawn,
	...
	try {
	    ...
	    report.reportHardFail(e, "Expand Applicant");
	}
    }
	...
	try {
	    ...
	    report.reportHardFail(e, "Focus on Created Product");
	}
    }
}
